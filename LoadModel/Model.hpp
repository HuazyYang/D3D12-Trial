...
struct ModelTexture {
  ...
  D3D12MAResourceSPtr defaultBuffer;
};
...
public:
  ...
  using TextureCollection           = std::vector<ModelTexture>;

  std::vector<std::shared_ptr<std::vector<D3D12_INPUT_ELEMENT_DESC>>> inputLayouts;
  ModelMesh::Collection meshes;
  ModelMaterialInfoCollection materials;
  TextureCollection texturesCache;
  std::wstring name;
  std::wstring filePath;

  HRESULT CreateFromSDKMESH(_In_ ResourceUploadBatch *pUploadBatch, _In_ const uint8_t *data,
                            _In_ size_t idataSize, _In_ ModelLoaderFlags flags);

  HRESULT CreateFromSDKMESH(_In_ ResourceUploadBatch *pUploadBatch, _In_ LPCWSTR pszFilePath,
