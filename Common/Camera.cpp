...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
void CBaseCamera::GetInput( bool bGetKeyboardInput, bool bGetMouseInput, bool bGetGamepadInput, WindowInteractor *pInteractor )
{
    ...
    if( bGetGamepadInput )
    {
        ...
        for( DWORD iUserIndex = 0; iUserIndex < DXUT::DXUT_MAX_CONTROLLERS; iUserIndex++ )
        {
            ...
            {
                m_GamePadLastActive[iUserIndex] = pInteractor->GetTotalTime();
            }
        }
...
//--------------------------------------------------------------------------------------
void CBaseCamera::UpdateMouseDelta(WindowInteractor *pInteractor)
{
    ...
    if( m_bResetCursorAfterMove && pInteractor->IsPaused() )
    {
        ...
        GetMonitorInfoW(MonitorFromWindow(pInteractor->GetHwnd(), MONITOR_DEFAULTTONEAREST), &mi);
...
//--------------------------------------------------------------------------------------
void CFirstPersonCamera::FrameMove( _In_ float fElapsedTime, WindowInteractor *pInteractor )
{
    if( pInteractor->IsPaused() )
    {
        if (pInteractor->GetFPS() == 0.0f)
            fElapsedTime = 0;
        else
            fElapsedTime = 1.0f / pInteractor->GetFPS();
    }
    ...
    GetInput( m_bEnablePositionMovement, ( m_nActiveButtonMask & m_nCurrentButtonMask ) || m_bRotateWithoutButtonDown, true, pInteractor );
...
//--------------------------------------------------------------------------------------
void CModelViewerCamera::FrameMove( _In_ float fElapsedTime, WindowInteractor *pInteractor )
{
    if( IsKeyDown( m_aKeys[CAM_RESET] ) )
        Reset();
    ...
    GetInput( m_bEnablePositionMovement, m_nCurrentButtonMask != 0, true, pInteractor );
