...
public:
    ...
    virtual void FrameMove( _In_ float fElapsedTime, _In_ WindowInteractor *pInteractor ) = 0;
...
protected:
    ...
    void UpdateMouseDelta(WindowInteractor *pInterator);
    void UpdateVelocity( _In_ float fElapsedTime );
    void GetInput( _In_ bool bGetKeyboardInput, _In_ bool bGetMouseInput, _In_ bool bGetGamepadInput, WindowInteractor *pInteractor );

    DirectX::XMFLOAT4X4 m_mView;                    // View matrix 
    DirectX::XMFLOAT4X4 m_mProj;                    // Projection matrix

    DXUT::DXUT_GAMEPAD m_GamePad[DXUT::DXUT_MAX_CONTROLLERS];  // XInput controller state
    DirectX::XMFLOAT3 m_vGamePadLeftThumb;
    DirectX::XMFLOAT3 m_vGamePadRightThumb;
    double m_GamePadLastActive[DXUT::DXUT_MAX_CONTROLLERS];
...
public:
    ...
    // Call these from client and use Get*Matrix() to read new matrices
    virtual void FrameMove( _In_ float fElapsedTime, _In_ WindowInteractor *pInteractor ) override;
